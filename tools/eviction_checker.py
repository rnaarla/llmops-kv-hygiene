        ...
        failures.append(
            {
                "metric": "coverage_pct",
                "actual": coverage,
                "expected_min": th.coverage_pct_min,
            }
        )
    if unsanitized > th.unsanitized_regions_max:
        failures.append(
            {
                "metric": "unsanitized_regions_count",
                "actual": unsanitized,
                "expected_max": th.unsanitized_regions_max,
            }
        )
        ...
        failures.append(
            {
                ...
                "actual": quarantine,
                "expected_max": th.quarantine_count_max,
            }
        )
        ...
        failures.append(
            {
                "metric": "reuse_rate",
                "actual": reuse_rate,
                "expected_max": th.reuse_rate_max,
            }
        )
    if th.sanitize_p95_ms_max is not None and p95 > th.sanitize_p95_ms_max:
        failures.append(
            {
                "metric": "sanitize_duration_p95_ms",
                "actual": p95,
                "expected_max": th.sanitize_p95_ms_max,
            }
        )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument(
        "metrics",
        type=str,
        help="Path to coverage metrics JSON exported by CacheTracer",
    )
